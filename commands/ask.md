...
```bash
"${CLAUDE_PLUGIN_ROOT}/bin/bridger" ask <peer> "<question>" --timeout 120 --json
```
...
- On timeout (exit 1), tell the user the peer did not respond — its session may not be listening (`/bridger:status` shows unread counts piling up).
